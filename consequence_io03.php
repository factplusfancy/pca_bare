...
if (isset($_POST['yes_confirm_post_1e'])) {
    ...
    // SPECIAL CASE: $_SESSION['Scratch']['t0consequence'] serves like $_SESSION['Selected']. $_SESSION['Selected'] keeps holding a t0pha row.
    // SPECIAL CASE: $_SESSION['Scratch']['t0subprocess'] and $_SESSION['Scratch']['t0scenario'] keep holding their selected rows.
    if (!isset($_SESSION['Scratch']['PlainText']['SecurityToken']) or $_SESSION['Scratch']['PlainText']['SecurityToken'] != 'pha_i1m.php' or !isset($_SESSION['Selected']['k0pha']) or !isset($_SESSION['Scratch']['t0subprocess']['k0subprocess']) or !isset($_SESSION['Scratch']['t0scenario']['k0scenario']) or !isset($_SESSION['Scratch']['t0consequence']['k0consequence']) or !isset($_SESSION['Post']) or !isset($_SESSION['t0user_practice']))
    ...
    echo $ccsaZfpf->ccsa_edit('consequence', $Zfpf);
...
if (!isset($_SESSION['Scratch']['PlainText']['SecurityToken']) or $_SESSION['Scratch']['PlainText']['SecurityToken'] != 'pha_i1m.php' or !isset($_SESSION['Selected']['k0pha']) or !isset($_SESSION['Scratch']['t0subprocess']) or !isset($_SESSION['Scratch']['t0scenario']) or !isset($_SESSION['t0user_practice']))
    $Zfpf->eject_1c(@$Zfpf->error_prefix_1c().__FILE__.':'.__LINE__);
if (!isset($_POST['consequence_o1']) and !isset($_POST['consequence_history_o1']) and $_SESSION['Selected']['k0pha'] < 100000 and $Zfpf->decrypt_1c($_SESSION['t0user']['c5app_admin']) != 'Yes')
    $Zfpf->send_to_contents_1c(__FILE__, __LINE__); // Don't eject
$htmlFormArray = $ccsaZfpf->html_form_array('consequence', $Zfpf);
$ccsaZfpf->ccsa_io0_2('consequence', $Zfpf, $htmlFormArray);
$Zfpf->save_and_exit_1c();
