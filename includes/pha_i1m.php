...
if ($RowsReturned['t0pha']) {
    ...
    if ($ProcessPHAsCount) {
        $Message .= '<p>
        <a class="toc" href="glossary.php#pha" target="_blank">PHA</a> for the currently selected process:</p><p>';
        ...
        $Message .= '<p><b>
        Nothing found</b> for the current process, neither draft nor issued. Please contact your supervisor if this seems amiss.</p><p>
...
if (!$ProcessPHAsCount) { // Below relevent even if !$RowsReturned['t0pha']
    if ($PracticePriv == MAX_PRIVILEGES_ZFPF) {
        if (!$RowsReturned['t0pha'])
            $Message .= '<p>
            No templates found. Ask an app admin to import one.</p>';
        $Message .= '<p>
    ...
    else // means $PracticePriv != MAX_PRIVILEGES_ZFPF
        $Message .= '<p><b>
        Nothing found</b> for the current process, neither draft nor issued. Please contact your supervisor if this seems amiss.</p><p><b>
        Practice Privileges Notice</b>: You have privileges to neither insert new templates nor create a PHA from a template. If you need these, please contact your supervisor or an app admin.</p>';
}
if ($AppAdmin == 'Yes' and $UserGlobalDBMSpriv == MAX_PRIVILEGES_ZFPF)
    $Message .= '<p>
    Import a template PHA, accessible to all users in this PSM-CAP App deployment with PHA-edit privileges, from a JSON file compatible with the PSM-CAP App PHA schema.<br />
    Maximum file size '.MAX_FILE_SIZE_ZFPF/1000000 .' MB (megabytes).<br />
        ...
        <input type="file" name="file_1_pha_json_file" /><br />
        <input type="submit" name="template_pha_import" value="Import" /></p>';
$Message .= '
</form>';
