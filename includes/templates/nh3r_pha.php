...
$pha = array(
    0 => array(
        ...
        'c6team_qualifications' => $Zfpf->encrypt_1c('[consulting firm name] assisted [client name] with completing this [year] process-hazard analysis (PHA). The PHA was led by [name, title, company] [describe leader\'s experience with the PHA method and the type of '.HAZSUB_PROCESS_NAME_ZFPF.' at the facility] [List qualifications of other PHA team members, such as design engineer, maintenance supervisor, mechanics, operators, health and safety manager, etc.]'),
        'c6bfn_attendance' => $EncryptedNothing, // Set as EncryptedNothing in setup.php
        'c6background' => $Zfpf->encrypt_1c('[Summary description of the '.HAZSUB_PROCESS_NAME_ZFPF.', quantity of chemical(s) it holds, rules that therefore require a PHA] [If applicable, provide history of past PHAs and PHA updates, revisions, or amendments.] [List dates of team meetings for latest PHA effort.]'),
        'c6method' => $Zfpf->encrypt_1c('The [year] PHA team used the [name and describe method, for example] what-if/checklist method. This is a hazard identification and risk analysis (HIRA) method intended to identify hazards and existing safeguards, provide a semi-quantitative risk assessment for each hazard, and make recommendations, and so satisfy the PHA requirements of the PSM and CAP rules applicable in the United States of America.

Prior to staring the PHA, the team leader verified that the process-safety information, possibly augmented by the PHA team\'s ability to observe the as-built '.HAZSUB_PROCESS_NAME_ZFPF.', was adequate to complete the PHA. 
        
During the PHA meetings, common as well as site-specific what-if questions (scenarios) were asked. Then, for each scenario, the PHA team identified or reviewed potential causes, previous incidents, consequences, existing or planned-before-startup engineering or administrative safeguards, severity, and likelihood of resulting in the agreed severity. The team also had the opportunity to identify recommendations for each scenario and to describe additional scenarios or prior incidents for the PHA team to consider.

Site and floor plans, the '.HAZSUB_PROCESS_NAME_ZFPF.' piping and instrumentation diagram, and other process-safety information were reviewed during the PHA meetings as needed to evaluate scenarios. The PHA was projected from a computer onto a screen during the PHA meetings, allowing the team members to review the PHA as the leader was typing the team\'s comments and recommendations into the PHA. A draft copy of the [year] PHA was also circulated to team members from [date] to [date]. 

Risk ranks for each scenario were assigned, based on the agreed severity and likelihood, using the risk-ranking matrix linked below.
...
[OPTIONAL FOR ADDITIONS OR NEW PLANTS] On [dates], prior to the preparation of a detailed design, pre-PHA meetings were held to assign responsibilities for completing the process-safety information and to look for any needed engineering controls that could be implemented more easily if identified before the detailed design. These meetings included [names, titles, organizations]. A draft PHA based on the this effort was circulated to the meeting participants as well as [NAMES, TITLES, AND COMPANIES]'),
        ...
        'c6nymd_leader' => $Zfpf->encrypt_1c('Template ammonia-refrigeration what-if/checklist PHA 2020-08-20') // Put template name here. Never overwritten because templates are never issued. (They may be edited)
    )
